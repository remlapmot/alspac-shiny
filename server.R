...
shinyServer(function(input, output, session) {
  output$variablecount <- renderUI({
    HTML(
      "<p>There are <strong>",
      nrow(dat),
      "</strong> variables that are currently available to researchers."
    )
  })

  output$laba <- renderUI({
    HTML(
      sapply(names(labs), function(x) {
        paste0(
          "<button id='category",
          which(names(labs) == x)[1],
          "' action='toggle' class='btn btn-default action-button btn-xs' style='margin-bottom: 3px'>",
          x,
          " (",
          labs[names(labs) == x],
          ")</button>"
